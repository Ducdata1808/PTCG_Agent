import os
import sys
import time
from collections import Counter

# Ensure root and submission folders are in path
root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, root_dir)
sys.path.insert(0, os.path.join(root_dir, "submission"))

from cg.game import battle_start, battle_select, battle_finish
from cg.api import to_observation_class, LogType
from main import agent as v3_agent
from models.v2 import agent as v2_agent
from main import read_deck_csv

def play_single_game(agent0, agent1, deck0, deck1):
    stats = {
        'winner': -1,
        'win_reason': 'Unknown',
        'turns': 0,
        'times': {0: [], 1: []},
        'damage_dealt': {0: 0, 1: 0},
        'cards_played': {0: 0, 1: 0},
        'cards_attached': {0: 0, 1: 0},
        'cards_evolved': {0: 0, 1: 0},
    }
    
    obs_dict, start_data = battle_start(deck0, deck1)
    if start_data.errorType != 0:
        battle_finish()
        return None

    obs = to_observation_class(obs_dict)
    
    try:
        while obs.current is None or obs.current.result == -1:
            current_player = obs.current.yourIndex if (obs.current is not None) else 0
            
            t0 = time.perf_counter()
            if current_player == 0:
                action = agent0(obs_dict)
            else:
                action = agent1(obs_dict)
            t1 = time.perf_counter()
            
            stats['times'][current_player].append(t1 - t0)
            obs_dict = battle_select(action)
            obs = to_observation_class(obs_dict)
            
            for log in obs.logs:
                if log.type == LogType.HP_CHANGE:
                    target_player = log.playerIndex
                    change_val = log.value if log.value else 0
                    if change_val < 0:
                        stats['damage_dealt'][1 - target_player] += abs(change_val)
                elif log.type == LogType.PLAY:
                    stats['cards_played'][log.playerIndex] += 1
                elif log.type == LogType.ATTACH:
                    stats['cards_attached'][log.playerIndex] += 1
                elif log.type == LogType.EVOLVE:
                    stats['cards_evolved'][log.playerIndex] += 1
                elif log.type == LogType.RESULT:
                    stats['winner'] = log.result
                    reason_map = {1: "Prize Out", 2: "Deck Out", 3: "Bench Out", 4: "Card Effect"}
                    stats['win_reason'] = reason_map.get(log.reason, f"Code {log.reason}")

            stats['turns'] += 1
            if stats['turns'] > 1000:
                stats['win_reason'] = "Timeout"
                break
                
        battle_finish()
        return stats
    except Exception as e:
        battle_finish()
        return None

def run_benchmark(num_games=10):
    print(f"=== RUNNING V3 (AlphaGo MCTS) vs V2 (Heuristic MCTS) BENCHMARK ({num_games} Games) ===")
    print("Agent 0: Version 3.0 (submission/main.py)")
    print("Agent 1: Version 2.0 (models/v2.py)\n")
    
    deck0 = read_deck_csv()
    deck1 = read_deck_csv()
    
    v3_wins = 0
    v2_wins = 0
    errors = 0
    
    win_reasons = Counter()
    total_turns = 0
    total_damage_dealt = {0: 0, 1: 0} # 0 for v3, 1 for v2
    total_played = {0: 0, 1: 0}
    total_attached = {0: 0, 1: 0}
    total_evolved = {0: 0, 1: 0}
    all_times_v3 = []
    all_times_v2 = []
    
    for i in range(num_games):
        sys.stdout.write(f"\rSimulating Game {i+1}/{num_games}...")
        sys.stdout.flush()
        
        if i % 2 == 0:
            stats = play_single_game(v3_agent, v2_agent, deck0, deck1)
            if stats and stats['winner'] == 0:
                v3_wins += 1
                win_reasons[stats['win_reason']] += 1
            elif stats and stats['winner'] == 1:
                v2_wins += 1
            else:
                errors += 1
            if stats:
                all_times_v3.extend(stats['times'][0])
                all_times_v2.extend(stats['times'][1])
                total_damage_dealt[0] += stats['damage_dealt'][0]
                total_damage_dealt[1] += stats['damage_dealt'][1]
                total_played[0] += stats['cards_played'][0]
                total_played[1] += stats['cards_played'][1]
                total_attached[0] += stats['cards_attached'][0]
                total_attached[1] += stats['cards_attached'][1]
                total_evolved[0] += stats['cards_evolved'][0]
                total_evolved[1] += stats['cards_evolved'][1]
        else:
            stats = play_single_game(v2_agent, v3_agent, deck1, deck0)
            if stats and stats['winner'] == 1:
                v3_wins += 1
                win_reasons[stats['win_reason']] += 1
            elif stats and stats['winner'] == 0:
                v2_wins += 1
            else:
                errors += 1
            if stats:
                all_times_v3.extend(stats['times'][1])
                all_times_v2.extend(stats['times'][0])
                total_damage_dealt[0] += stats['damage_dealt'][1]
                total_damage_dealt[1] += stats['damage_dealt'][0]
                total_played[0] += stats['cards_played'][1]
                total_played[1] += stats['cards_played'][0]
                total_attached[0] += stats['cards_attached'][1]
                total_attached[1] += stats['cards_attached'][0]
                total_evolved[0] += stats['cards_evolved'][1]
                total_evolved[1] += stats['cards_evolved'][0]
                 
        if stats:
            total_turns += stats['turns']

    total_valid = v3_wins + v2_wins
    win_rate = (v3_wins / total_valid * 100) if total_valid > 0 else 0
    
    print("\n\n" + "="*41)
    print("           BENCHMARK RESULTS           ")
    print("="*41)
    print(f"V3 (AlphaGo Neural MCTS) Wins : {v3_wins} ({win_rate:.2f}%)")
    print(f"V2 (Heuristic MCTS) Wins      : {v2_wins} ({100 - win_rate:.2f}%)")
    print(f"Errors / Timeouts             : {errors}")
    print(f"Average Turn Length           : {total_turns/num_games if num_games > 0 else 0:.1f} actions/game")
    
    print("\n--- VICTORY TYPE BREAKDOWN (V3) ---")
    for reason, count in win_reasons.items():
        pct = count / v3_wins * 100 if v3_wins > 0 else 0
        print(f"* {reason}: {count} games ({pct:.1f}%)")
        
    print("\n--- AVERAGE METRICS PER GAME (V3 vs V2) ---")
    print(f"Damage Dealt     : {total_damage_dealt[0]/num_games:.1f} vs {total_damage_dealt[1]/num_games:.1f}")
    print(f"Cards Played     : {total_played[0]/num_games:.1f} vs {total_played[1]/num_games:.1f}")
    print(f"Energy Attached  : {total_attached[0]/num_games:.1f} vs {total_attached[1]/num_games:.1f}")
    print(f"Evolutions Done  : {total_evolved[0]/num_games:.1f} vs {total_evolved[1]/num_games:.1f}")
    
    print("\n--- DECISION TIMINGS ---")
    for name, times in [("V3 Agent", all_times_v3), ("V2 Agent", all_times_v2)]:
        if times:
            avg_time = sum(times) / len(times) * 1000
            max_time = max(times) * 1000
            print(f"* {name}: Avg={avg_time:.2f}ms, Max={max_time:.2f}ms")
    print("="*41)

if __name__ == "__main__":
    run_benchmark(num_games=10)
